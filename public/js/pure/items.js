...
export function pickDangerIndex(dangerRand) {
    return (Math.floor(dangerRand * 3) + 1);
}
/**
 * 次に出す食材を決める純粋版。
 ...
 */
export function pickNextItem(rushMode, rand, dangerRand) {
    if (rushMode)
        return ITEMS[0];
    if (rand < EEL_RATE)
        return ITEMS[0];
    if (rand < DANGER_RATE_END)
        return ITEMS[pickDangerIndex(dangerRand)];
    return ITEMS[4];
}
/** 仕分けの正誤表。pan-eel / trash-danger / dolphin-dolphin が正解。 */
export function isCorrectSort(itemType, chosenType) {
    if (chosenType === "pan")
        return itemType === "eel";
    if (chosenType === "trash")
        return itemType === "danger";
