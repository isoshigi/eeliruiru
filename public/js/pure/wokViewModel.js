...
export function getWokBadgeText(eelCount) {
    return `${eelCount}/5 匹`;
}
export function getPullBonusText(eelCount) {
    return PULL_BONUS_TEXTS[eelCount] ?? EMPTY_PULL_BONUS_TEXT;
}
/** 旧実装の status 分岐（raw/perfect/burning + 空鍋）をそのままデータ化。 */
export function getWokStatusView(status, eelCount) {
