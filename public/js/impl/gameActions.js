...
import { isCorrectSort, pickNextItem } from "../pure/items.js";
import { applyMissPenalty, calcMissPenalty, calcPullPoints, calcRawPullPoints, getRankTitle, shouldTriggerFever, } from "../pure/scoring.js";
...
export function startGame() {
    ...
    gameState.timerInterval = setInterval(() => {
        ...
        gameState.timeLeft--;
        if (els.timer)
            els.timer.innerText = String(gameState.timeLeft);
...
export function nextItem() {
    ...
    if (els.itemCard)
        void els.itemCard.offsetWidth;
    els.itemCard?.classList.add("shake-heavy");
}
export function actionSort(chosenType) {
...
export function updateWokCooking() {
    if (!gameState.isPlaying || gameState.wok.eelCount === 0)
        return;
...
export function addScore(pts) {
    gameState.score += pts;
    if (els.score)
        els.score.innerText = String(gameState.score);
}
export function addCombo() {
    gameState.combo++;
    if (els.comboCount)
        els.comboCount.innerText = String(gameState.combo);
...
export function handleMiss(reason) {
    sfx.miss();
    gameState.combo = 0;
...
export function triggerRushMode() {
    if (gameState.rushMode || gameState.isTutorial)
        return;
