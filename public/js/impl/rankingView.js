...
export let rankingScope = "daily";
export function setRankingScope(scope) {
    rankingScope = scope;
}
export function renderRankingRows(rankings) {
...
export function updateRankingTabs() {
...
export async function renderStartRanking() {
    updateRankingTabs();
...
export function switchRankingScope(scope) {
...
export function checkAndHandleHighScore(score, rankTitle) {
    ...
    $("btn-save-score").onclick = async () => {
        ...
        btn.disabled = true;
        try {
            const name = sanitizePlayerName($("player-name-input").value);
