...
export function setTutStep(n: number): void {
  tutStep = n;
}

interface TutStepDef {
  stepText: string;
  icon: string;
  title: string;
  desc: string;
  highlightElId: string;
  itemToSet: Item;
}

export const tutSteps: TutStepDef[] = [
  {
    stepText: "STEP 1 / 4",
    icon: "🐍",
    title: "「新鮮ウナギ」を鍋に入れる！",
...
export function startTutorialMode(): void {
  ...
  if (els.timer) els.timer.innerText = "∞";
...
export function applyTutStep(stepIdx: number): void {
  clearAllHighlights();
...
export function endTutorialMode(): void {
  clearAllHighlights();
