...
export function stepParticle(p: ParticleState): ParticleState {
  return {
    ...p,
    x: p.x + p.vx,
    y: p.y + p.vy,
...
export function particleAlpha(life: number, maxLife: number): number {
  return Math.max(0, life / maxLife);
}

/** 消滅判定。旧実装: life <= 0 で splice */
export function isParticleDead(life: number): boolean {
  return life <= 0;
}
 ...
 */
export function makeExplosionSpecs(
  x: number,
  y: number,
